...
func mergeDeviceVotesToUser(db *sqlx.DB, userID uint, deviceID string) error {
	...
	for _, vote := range votes {
		...
		if err == nil {
			...
			// Pick rows decrement the previous item's counter; skip rows
			// have no item reference, so we skip the decrement and just
			// delete the anon row.
			if anonItem := vote.PickedItemID(); anonItem != "" {
				var previousItem models.MatchupItem
				if err := sqlx.GetContext(context.Background(), tx, &previousItem,
					"SELECT * FROM matchup_items WHERE public_id = $1", anonItem); err != nil {
					return err
				}
		...
		// Reassign vote (pick or skip) to the new user. The kind /
		// item-reference is preserved as-is; only the identity flips.
		if _, err := tx.ExecContext(context.Background(),
